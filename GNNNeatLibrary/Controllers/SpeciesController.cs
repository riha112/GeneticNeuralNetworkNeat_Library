...
namespace GNNNeatLibrary.Controllers
{
    public class SpeciesController
    {
        ...
        /// <returns></returns>
        public bool IsSameFamily(ref NetModel left, ref NetModel right)
        {
            ...
            if (left.Connections.Count > right.Connections.Count)
                return IsSameFamily(ref right, ref left);


            var N = right.Connections.Count;
            if (N < 5) N = 5; // Spreads networks into more species if they are small.
